 ...
 * <p>
 * The {@link #internalJavaDumpToFile()}, {@link #systemDumpToFile()}, {@link #heapDumpToFile()} and
 ...
 * <br>
 * The recommended usage of the {@link #internalJavaDumpToFile()}, {@link #systemDumpToFile()},
...
public class DumpImpl {
	 ...
	 */
	protected static void internalJavaDump() {
	 ...
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
	protected static void internalHeapDump() {
		checkLegacySecurityPermssion();
		HeapDumpImpl();
	}
	 ...
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
	protected static void internalSystemDump() {
		checkLegacySecurityPermssion();
		SystemDumpImpl();
	}
	 ...
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
    protected static void internalSnapDump() {
    	checkLegacySecurityPermssion();
    	SnapDumpImpl();
    }
    
	private static final DumpPermissionImpl DUMP_PERMISSION = new DumpPermissionImpl();
	private static final ToolDumpPermissionImpl TOOL_DUMP_PERMISSION = new ToolDumpPermissionImpl();
	 ...
	 * @throws InvalidDumpOptionExceptionBase  if the filename was invalid
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
    protected static String internalJavaDumpToFile(String fileNamePattern ) throws InvalidDumpOptionExceptionBase {
    	String request = null;
    	...
    	String dump = null;
    	dump = internalTriggerDump(request, "javaDumpToFile"); //$NON-NLS-1$
     	return dump;
    }
	 ...
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump 
	 */
    protected static String internalJavaDumpToFile() {
    	try {
    		return internalTriggerDump("java", "javaDumpToFile"); //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
    protected static String internalHeapDumpToFile(String fileNamePattern ) throws InvalidDumpOptionExceptionBase {
     	String request = null;
    	if( "".equals(fileNamePattern) ) { //$NON-NLS-1$
    		fileNamePattern = null;
    	}
    	if( fileNamePattern != null ) {
   		...
   		String dump = null;
   		dump = internalTriggerDump(request, "heapDumpToFile"); //$NON-NLS-1$
    	return dump;
    }
	 ...
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
    protected static String internalHeapDumpToFile() {
    	try {
    		return internalTriggerDump("heap:opts=PHD", "heapDumpToFile"); //$NON-NLS-1$ //$NON-NLS-2$
    	} catch (InvalidDumpOptionExceptionBase e) {
    		// Cannot actually be thrown, since we aren't specifying anything other than a known dump type.
    		// Prevent the user having to add a catch block to their call.
    		return null;
	 ...
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
    protected static String internalSystemDumpToFile(String fileNamePattern) throws InvalidDumpOptionExceptionBase {
    	String request = null;
    	if( "".equals(fileNamePattern) ) { //$NON-NLS-1$
    		fileNamePattern = null;
    	}
    	...
    	String dump = null;
       	dump = internalTriggerDump(request, "systemDumpToFile"); //$NON-NLS-1$
       	return dump;
    }
	 ...
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
    protected static String internalSystemDumpToFile() {
     	try {
    		return internalTriggerDump("system", "systemDumpToFile"); //$NON-NLS-1$ //$NON-NLS-2$
    	} catch (InvalidDumpOptionExceptionBase e) {
    		// Cannot actually be thrown, since we aren't specifying anything other than a known dump type.
    		// Prevent the user having to add a catch block to their call.
    		return null;
     ...
     * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
    protected static String internalSnapDumpToFile(String fileNamePattern) throws InvalidDumpOptionExceptionBase {
    	String request = null;
    	if( "".equals(fileNamePattern) ) { //$NON-NLS-1$
    		fileNamePattern = null;
    	}
    	...
    	String dump = null;
    	dump = internalTriggerDump(request, "snapDumpToFile"); //$NON-NLS-1$
    	return dump;
    }
	 ...
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 */
    protected static String internalSnapDumpToFile() {
    	try {
    		return internalTriggerDump("snap", "snapDumpToFile"); //$NON-NLS-1$ //$NON-NLS-2$
    	} catch (InvalidDumpOptionExceptionBase e) {
    		// Cannot actually be thrown, since we aren't specifying anything other than a known dump type.
    		// Prevent the user having to add a catch block to their call.
    		return null;
    ...
    private static void checkForExtraOptions(String fileNamePattern) throws InvalidDumpOptionExceptionBase {
		// Check no-one has tried to sneak options onto the end of a filename.
    	if (fileNamePattern.contains(",")) { //$NON-NLS-1$
    		throw new InvalidDumpOptionExceptionBase("Invalid dump filename specified."); //$NON-NLS-1$
    	}
	}
     ...
     * @throws InvalidDumpOptionExceptionBase  If the dump options are invalid or the dump operation fails
	 * @throws RuntimeException if the vm does not contain RAS dump support
	 * @throws SecurityException if there is a security manager and it doesn't allow the checks required to trigger this dump
	 * @throws NullPointerException if dumpSettings is null
	 */
	protected static String internalTriggerDump(String dumpOptions) throws InvalidDumpOptionExceptionBase {
		if( dumpOptions == null ) {
		...
		return internalTriggerDump(dumpOptions, "triggerDump"); //$NON-NLS-1$
	}

	protected static String internalTriggerDump(String dumpSettings, String event) throws InvalidDumpOptionExceptionBase {
	 ...
	 * @throws InvalidDumpOptionExceptionBase if the specified option cannot be set or is incorrect
     * @throws DumpConfigurationUnavailableExceptionBase If the dump configuration cannot be changed because a dump is currently in progress
     ...
     */
    protected static void internalSetDumpOptions(String dumpOptions) throws InvalidDumpOptionExceptionBase, DumpConfigurationUnavailableExceptionBase {	
     ...
     */
    protected static String[] internalQueryDumpOptions() {
     ...
     * @throws SecurityException if there is a security manager and it doesn't allow the checks required to change the dump settings
     */
    protected static void internalResetDumpOptions() throws DumpConfigurationUnavailableExceptionBase {
